...
%>	
		<INPUT CLASS="BirtViewer_parameter_dialog_Input"
			...
			<%if (encodedParameterName.indexOf("startDate")>=0) {%> onClick="WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'endDate\',{d:-1})}'})"  class="Wdate" <%} %>
			<%if (encodedParameterName.indexOf("endDate")>=0) {%>    onClick="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'startDate\',{d:1})}'})" class="Wdate" <%} %>
